...
void loop() {
  ...
  Serial.println(F(" *C"));
  ave[i]=bme.temperature;
  
  i++;
  if(i=60){ for(int j=0;j<=60;j--){ sum =sum+ ave[j];}}
  if (i>60){i=0; mean=sum/60;}
...
void sendMessage() {
  String msg = String(bme.temperature);
  /*String msg = "Hello from node bme ";
  msg += mesh.getNodeId();
  msg += " myFreeMemory: " + String(ESP.getFreeHeap());
  msg += " Temprature:" + String(bme.temperature) + "*C";
